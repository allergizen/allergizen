...
import { Text, View, StyleSheet, Button, TouchableOpacity, Image, Dimensions, ScrollView } from 'react-native';
...
export default function App() {
      ...
      return <Text>
         {allergeni.map((el, i) => {
            if (userAllergens.includes(el))
               return <><Text style={{color: color, fontWeight: 'bold'}}>{el}</Text>{i + 1 === allergeni.length? '' : ', '}</>
            else
               return <>{el}{i + 1 === allergeni.length? '' : ', '}</>
         })}
      </Text>
   }
   ...
   return (
      <View style={[styles.container, {backgroundColor: 'black'}]}>
         ...
         <Animatable.View animation={snapAnim} style={[styles.result, {top: resultY}]} duration={500}>
            <View style={styles.dragHitBox} onStartShouldSetResponder={() => {setSnapAnim(null); return () => true}} onResponderMove={handleDrag} onResponderRelease={handleRelease}><View style={styles.dragBar}></View></View>
            
            {(dati === null)? <></> : <>
               <View style={styles.product}>
                  <Image source={{uri: dati.product.image_url}} style={styles.img} resizeMode="contain"></Image>
                  <View style={styles.info}>
                     <Text style={{fontSize: 40, fontWeight: 'bold', marginBottom: -5}}>{dati.product.product_name}</Text>
                     ...
                     <TouchableOpacity style={styles.savebutton} onPress={() => alert('Aggiunto ai preferiti!')}>
                        <AddIcon></AddIcon><Text>Aggiungi ai preferiti</Text>
                     </TouchableOpacity>
                  </View>
               </View>
               
               <Text style={styles.infoHeader}>Allergeni</Text>
               <Text style={styles.infoContent}>{scriviAllergeni(dati.product.allergens, 'red')}</Text>
               <Text style={[styles.infoHeader, {top: 20}]}>Ingredienti</Text>
               <Text style={[styles.infoContent, {top: 20}]}>{dati.product.ingredients_text === ''? 'Non trovati ' : dati.product.ingredients_text}</Text>
               <Text style={[styles.infoHeader, {top: 40}]}>Tracce</Text>
               <Text style={[styles.infoContent, {top: 40}]}>{dati.product.traces === ''? 'Niente' : scriviAllergeni(dati.product.traces, 'gold')}</Text>
               ...
               <View style={styles.nutriScore}>
                  ...
                  <NutriscoreA style={{transform: 'scale(0.7)', top: 15, left: -17}} id={dati.product.nutrition_grades != null ? dati.product.nutrition_grades : 'unknown'}></NutriscoreA>
                  <View style={styles.nutriScoreInfo}>
                     <Text style={{fontSize: 25, color: 'orange', fontWeight: 'bold'}}>Nutri-Score {dati.product.nutrition_grades != null ? dati.product.nutrition_grades.toUpperCase() : 'sconosciuto'}</Text>
