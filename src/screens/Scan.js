...
const ScanBorderTopRight = (props) => <Svg
  viewBox='0 0 110 110'
  height={35}
  width={35}
  fill='none'
...
const ScanBorderBottomLeft = (props) => <Svg
  viewBox='0 0 110 110'
  height={35}
  width={35}
  fill='none'
      ...
      <Text>
        {allergeni.map((el, i) => {
          if (userAllergens.includes(el))
            return <>
              <Text key={el} style={{ color: color, fontWeight: 'bold' }}>{el}</Text>
      ...
      >
        ...
        {dati === null ? <></> : <>
          ...
          <View style={styles.resultview}
            backgroundColor={api.get_allergens(dati.product.allergens).length != 0 ? Colors_.lightRed : api.get_allergens(dati.product.traces).length === 0 ? Colors_.lightGreen : Colors_.lightYellow}
          >
          ...
          <View style={[styles.nutriScore, { backgroundColor: nutriColor[dati.product.nutrition_grades ?? 'unknown'].color + '33' }]}>
            ...
            <View style={styles.nutriScoreInfo}>
              <Text style={{ fontSize: 25, color: nutriColor[dati.product.nutrition_grades ?? 'unknown'].color, fontWeight: 'bold' }}>
                {'Nutri-Score ' + ((dati.product.nutrition_grades !== null) ? dati.product.nutrition_grades.toUpperCase() : '?')}
              </Text>
              <Text>
                {'Qualità nutrizionale ' + nutriColor[dati.product.nutrition_grades ?? 'unknown'].text}
              </Text>
            </View>
          </View>
        </>}
